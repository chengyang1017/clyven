...
class SubtitleCueDragController {
  ...
  void beginResizeCue(SubtitleCueDetail detail, {required bool resizeStart}) {
    ...
    final track = html.document.getElementById('subtitle-timeline-track');
    ...
    _moveSubscription = html.document.onMouseMove.listen((moveEvent) {
      ...
      final targetMs = (relativeX / rect.width * durationMs).round();

      if (resizeStart) {
        detail.cue.startMs = targetMs.clamp(0, detail.cue.endMs - 100).toInt();
      } else {
        detail.cue.endMs = targetMs
            .clamp(detail.cue.startMs + 100, durationMs)
            .toInt();
      }
    ...
    _upSubscription = html.document.onMouseUp.listen((_) async {
      _cancelCurrentDrag();
  ...
  void beginMoveCue(SubtitleCueDetail detail) {
    final cueId = detail.cue.id;
    ...
    _moveSubscription = html.document.onMouseMove.listen((moveEvent) {
      ...
      final deltaMs = (deltaX / rect.width * durationMs).round();

      final maxStart = durationMs - cueDuration;

      final newStart = (originalStart + deltaMs).clamp(0, maxStart).toInt();
