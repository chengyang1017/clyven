...
class SubtitleTimeline extends StatelessComponent {
  ...
  final void Function(SubtitleCueDetail detail, bool resizeStart) onResizeStart;

  final String Function(SubtitleCueDetail detail) getCueText;

  final void Function(SubtitleCueDetail detail, String value) onCueTextChanged;

  final void Function(SubtitleCueDetail detail) onCueTextSave;
  ...
  Component build(BuildContext context) {
    ...
    return div(classes: 'subtitle-timeline-panel', [
        ...
        div(classes: 'subtitle-timeline-controls', [
          button(
            ...
            onClick: zoom <= 1.0
                ? null
                : () {
                    onZoomChanged((zoom / 1.5).clamp(1.0, 240.0).toDouble());
                  },
            [.text('−')],
          ),
          span([
      ...
      div(classes: 'subtitle-timeline', [
        div(
          classes: 'subtitle-timeline-ruler',
          attributes: {'style': 'width: ${zoom * 100}%;'},
          [
            for (var tickMs = 0; tickMs <= durationMs; tickMs += tickIntervalMs)
              div(
                classes: 'subtitle-timeline-tick',
                attributes: {'style': 'left: ${_timelinePercent(tickMs)}%;'},
                [
                  span(classes: 'subtitle-timeline-tick-line', []),
                  span(classes: 'subtitle-timeline-tick-label', [
              ...
              div(
                ...
                events: {
                  'mousedown': (event) {
                    ...
                    if ((event.target as dynamic)?.tagName
                            ?.toString()
                            .toLowerCase() ==
                        'input') {
                      return;
                ...
                [
                  ...
                  input<String>(
                    ...
                    attributes: {
                      ...
                      'aria-label': '直接编辑字幕',
                      if (selectedCueId != detail.cue.id)
                        'readonly': 'readonly',
