...
class SubtitleKaraokeController {
  ...
  List<KaraokeSegmentDraft> draftsFor(SubtitleCueDetail detail) {
    ...
    if (!hydratedCueIds.contains(cueId)) {
      final serverSegments = List<SubtitleKaraokeSegment>.from(
      ...
      draftsByCueId[cueId] = [
        for (final segment in serverSegments)
          KaraokeSegmentDraft(
            text: segment.text,
            startTime: formatSubtitleTime(segment.startOffsetMs),
            endTime: formatSubtitleTime(segment.endOffsetMs),
          ),
      ];
  ...
  void initializeFromSubtitle(SubtitleCueDetail detail, String sourceText) {
    final cueId = detail.cue.id;
  ...
  List<String> _piecesFromSource(SubtitleCueDetail detail, String source) {
    ...
    if (tokens.isNotEmpty) {
      ...
      for (final token in tokens) {
        ...
        final index = source.indexOf(tokenText, cursor);
        ...
        pieces.add(source.substring(cursor, end));
      ...
      if (matched && pieces.isNotEmpty) {
        if (cursor < source.length) {
          pieces[pieces.length - 1] =
              '${pieces.last}${source.substring(cursor)}';
        }
  ...
  void addSegment(SubtitleCueDetail detail) {
    final cueId = detail.cue.id;
  ...
  void removeSegment(SubtitleCueDetail detail, int index) {
    final cueId = detail.cue.id;
  ...
  void clear(SubtitleCueDetail detail) {
    final cueId = detail.cue.id;
  ...
  void updateText(SubtitleCueDetail detail, int index, String value) {
    _updateDraft(detail, index, (draft) {
      draft.text = value;
    });
  }

  void updateStart(SubtitleCueDetail detail, int index, String value) {
    _updateDraft(detail, index, (draft) {
      draft.startTime = value;
    });
  }

  void updateEnd(SubtitleCueDetail detail, int index, String value) {
    _updateDraft(detail, index, (draft) {
      draft.endTime = value;
    });
  }
  ...
  void useCurrentTimeAsStart(
    ...
    final relativeMs = _relativeTime(detail, absoluteVideoMs);

    updateStart(detail, index, formatSubtitleTime(relativeMs));
  }
  ...
  void useCurrentTimeAsEnd(
    ...
    updateEnd(detail, index, formatSubtitleTime(relativeMs));
  }

  int _relativeTime(SubtitleCueDetail detail, int absoluteVideoMs) {
    final durationMs = detail.cue.endMs - detail.cue.startMs;

    return (absoluteVideoMs - detail.cue.startMs).clamp(0, durationMs).toInt();
  }

  Future<void> save(SubtitleCueDetail detail) async {
    final cueId = detail.cue.id;
