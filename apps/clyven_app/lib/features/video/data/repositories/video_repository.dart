...
abstract class VideoRepository {
  Future<VideoDetail> loadVideoDetail(String videoId);
...
class MockVideoRepository implements VideoRepository {
  final Map<String, VideoDetail> _createdVideos = {};

  final Map<String, String> _videoOwners = {};
  ...
  Future<VideoDetail> createVideo({
    ...
    final id = 'uploaded-${DateTime.now().microsecondsSinceEpoch}';
    ...
    _createdVideos[id] = video;

    _videoOwners[id] = userId;
  ...
  Future<List<VideoDetail>> loadUserVideos({required String userId}) async {
    ...
    final videos = _createdVideos.values.where((video) {
      return _videoOwners[video.id] == userId;
    }).toList();
  ...
  Future<VideoDetail> loadVideoDetail(String videoId) async {
    // 先检查用户刚刚发布的视频。
    final createdVideo = _createdVideos[videoId];
    ...
    switch (videoId) {
      ...
      case 'video-002':
        return VideoDetail(
          ...
          authorName: '未完成实验室',
          category: '开发手记',
          tags: const ['Flutter', '编程', 'Riverpod'],
          coverUrl:
