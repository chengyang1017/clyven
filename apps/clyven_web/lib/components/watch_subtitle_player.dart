...
class WatchSubtitlePlayer extends StatefulComponent {
  const WatchSubtitlePlayer({
    ...
    required this.onWideModeToggle,
    required this.onDisplayModeToggle,
  ...
  final void Function() onDisplayModeToggle;
  final void Function() onWideModeToggle;
...
class _WatchSubtitlePlayerState extends State<WatchSubtitlePlayer> {
  ...
  bool _adaptiveReady = false;
  bool _qualityMenuOpen = false;
  bool _hlsListenersAttached = false;
  ...
  void dispose() {
    ...
    html.document.dispatchEvent(
      html.CustomEvent(
        'clyven-hls-destroy',
        detail: <String, Object?>{'videoId': _videoElementId},
      ),
    );
  ...
  void _startAdaptivePlayback() {
    ...
    _adaptivePollTimer = Timer.periodic(
      const Duration(seconds: 10),
      (_) => _pollAdaptiveSource(),
    );
  }

  Future<void> _pollAdaptiveSource() async {
    if (_hlsManifestUrl != null) return;

    try {
      final manifestUrl = await webClient.video.getPlaybackManifestUrl(
        videoId: component.videoId,
      );

      if (!mounted || manifestUrl == null || manifestUrl.trim().isEmpty) {
        return;
      }

      _adaptivePollTimer?.cancel();
      _hlsManifestUrl = manifestUrl.trim();

      Future<void>.delayed(const Duration(milliseconds: 50), () {
        if (!mounted || _hlsManifestUrl == null) return;
        _attachHls(_hlsManifestUrl!);
      });
    } catch (_) {
      // Backend may not have the endpoint yet until deployment.
      // Keep original playback working and retry quietly.
    }
  }

  void _handleHlsReady(html.Event event) {
    if (!mounted) return;
  ...
  void _handleHlsUnavailable(html.Event event) {
    if (!mounted) return;

    setState(() {
      _adaptiveReady = false;
      _qualityMenuOpen = false;
      _selectedQualityHeight = null;
    });
  }

  void _attachHls(String manifestUrl) {
    final video = _video;

    if (video == null) {
      Future<void>.delayed(const Duration(milliseconds: 100), () {
        if (mounted) {
          _attachHls(manifestUrl);
        }
      });
      return;
    }

    if (!_hlsListenersAttached) {
      video.addEventListener('clyven-hls-ready', _handleHlsReady);
      video.addEventListener('clyven-hls-unavailable', _handleHlsUnavailable);

      _hlsListenersAttached = true;
    }
    ...
    html.document.dispatchEvent(
      html.CustomEvent(
        ...
        detail: <String, Object?>{
          'videoId': _videoElementId,
          'url': manifestUrl,
          'fallbackUrl': component.videoUrl,
        },
      ),
    );
  }
  ...
  void _setQuality(int? height) {
    html.document.dispatchEvent(
      html.CustomEvent(
        'clyven-hls-quality',
        detail: <String, Object?>{'videoId': _videoElementId, 'height': height},
      ),
    );

    setState(() {
      _selectedQualityHeight = height;
      _qualityMenuOpen = false;
    });
  }

  Component _qualityOption(int? height, String label) {
    final selected = _selectedQualityHeight == height;

    return button(
      type: ButtonType.button,
      classes:
          'watch-quality-option'
          '${selected ? ' is-active' : ''}',
      onClick: () => _setQuality(height),
      [.text(label)],
    );
  }
  ...
  Component _qualityControl() {
    ...
    return div(classes: 'watch-quality-control', [
      button(
        type: ButtonType.button,
        classes: 'watch-player-pill-button watch-quality-button',
        onClick: () {
          setState(() {
