...
class _SubtitlesPageState extends State<SubtitlesPage> {
  ...
  Future<void> _loadVideos() async {
    ...
    try {
      final result = await client.admin.getAllVideos();
      setState(() {
        videos = result;
        loading = false;
  ...
  Component build(BuildContext context) {
    ...
    return div(classes: 'subtitle-hub-v2', [
      div(classes: 'subtitle-hub-hero', [
        div([
          p(classes: 'subtitle-hub-kicker', [.text('CLYVEN REVIEW')]),
          h1([.text('Subtitle Review')]),
      ...
      if (loading)
        ...
        div(classes: 'subtitle-hub-grid', [
          section(classes: 'subtitle-hub-section subtitle-video-section', [
          ...
          section(classes: 'subtitle-hub-section subtitle-language-section', [
            _sectionHeader(
              '02',
              '选择语言',
              '直接读取 Glyphora 共享语言库，不再手填 language code。',
            ),
          ...
          section(classes: 'subtitle-hub-section subtitle-script-section', [
                ...
                for (final script in scripts)
                  button(
                    ...
                    [
                      ...
                      strong([.text(language.scriptNameOf(script.code, 'zh'))]),
                      span([.text(script.isRtl ? 'RTL · 从右到左' : 'LTR · 从左到右')]),
                    ],
                  ),
              ]),
            if (language != null && language.hasVariants)
              div(classes: 'subtitle-variant-note', [
                ...
                div([
                  ...
                  small([
                    .text(
                      '当前 cy 后端轨道查询仍以 languageCode 为主，因此这里先展示，不伪造 variant 路由。',
                    ),
                  ]),
                ]),
              ]),
