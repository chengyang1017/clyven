...
class DictionaryImportEndpoint extends Endpoint {
  ...
  Future<List<DictionaryImportProfile>> getProfiles(
    ...
    return DictionaryImportProfile.db.find(
      ...
      where: languageCode == null
          ? null
          : (p) => p.languageCode.equals(languageCode),
      orderBy: (p) => p.name,
    );
  }

  Future<DictionaryImportProfileDetail?> getProfile(
    Session session, {
    required int profileId,
  }) async {
    final profile = await DictionaryImportProfile.db.findById(
      session,
      profileId,
    );
    ...
    final mappings = await DictionaryImportMapping.db.find(
      session,
  ...
  Future<DictionaryImportProfileDetail> createVietnameseVocabularyProfile(
    Session session,
  ) async {
    final existing = await DictionaryImportProfile.db.findFirstRow(
      session,
      where: (p) =>
    ...
    if (existing != null) {
      var mappings = await DictionaryImportMapping.db.find(
        session,
        where: (m) => m.profileId.equals(existing.id),
        orderBy: (m) => m.position,
      );
      ...
      mappings = await DictionaryImportMapping.db.find(
        session,
        where: (m) => m.profileId.equals(existing.id),
        orderBy: (m) => m.position,
      );
    ...
    final profile = await DictionaryImportProfile.db.insertRow(
      session,
      DictionaryImportProfile(
        ...
        description: '越南语国语字、喃字、中文释义与多文字例句导入模板',
      ),
    );
    ...
    final insertedMappings = await DictionaryImportMapping.db.insert(
      session,
      mappings,
    );
  ...
  Future<DictionaryImportPreview?> previewRows(
    ...
    final previewRows = <DictionaryImportPreviewRow>[];
    ...
    for (var i = 0; i < rowsJson.length; i++) {
      ...
      try {
        ...
        final originalRowNumber = rawRow['__rowNumber'];
      ...
      final entry = <String, dynamic>{
        ...
        'entryType': profile.defaultEntryType ?? 'word',
        'primaryScriptCode': profile.primaryScriptCode,
      };
      ...
      final morphology = <Map<String, dynamic>>[];

      final exampleGroups = <String, List<Map<String, dynamic>>>{};
      ...
      for (final mapping in mappings) {
        ...
        switch (mapping.targetType) {
          ...
          case 'form':
            forms.add({
              'field': mapping.targetField ?? 'text',
              'text': value,
              'languageCode': mapping.languageCode ?? profile.languageCode,
              'scriptCode': mapping.scriptCode,
            });
          ...
          case 'definition':
            definitions.add({
              'field': mapping.targetField ?? 'gloss',
              'value': value,
          ...
          case 'example':
            final groupKey = mapping.groupKey ?? 'example_default';

            final group = exampleGroups.putIfAbsent(
              groupKey,
              () => <Map<String, dynamic>>[],
            );
          ...
          case 'morphology':
            morphology.add({
              'field': mapping.targetField ?? mapping.sourceColumn,
              'value': value,
            });
      ...
      final headword = cleanValue(entry['text']);
      ...
      if (missingRequired.isNotEmpty) {
        status = 'warning';
        message = '词条尚未完成：缺少 ${missingRequired.join(', ')}；正式导入时会跳过';
        warningRows++;
      } else if (headword == null) {
        status = 'warning';
        message = '词条尚未完成：没有主词；正式导入时会跳过';
        warningRows++;
      } else {
        final entryType = entry['entryType']?.toString() ?? 'word';

        final duplicateKey = '$entryType|${headword.toLowerCase()}';

        final firstRow = seenHeadwordRows[duplicateKey];
      ...
      final normalized = {
        ...
        'examples': [
          for (final item in exampleGroups.entries)
            {
              'groupKey': item.key,
              'texts': item.value,
            },
  ...
  Future<DictionaryImportPreview?> previewExcelBase64(
    ...
    final sheet = workbook.tables[profile.sheetName];
    ...
    final headers = [
      for (final cell in headerRow) cell?.value?.toString().trim() ?? '',
    ];
    ...
    for (var rowIndex = 1; rowIndex < sheet.rows.length; rowIndex++) {
      ...
      for (var columnIndex = 0; columnIndex < headers.length; columnIndex++) {
        ...
        final cell = columnIndex < excelRow.length
