  ...
  static Future<DictionaryImportCommitResult> commit(
    ...
    for (final row in preview.rows) {
      ...
      try {
        ...
        final languageCode =
            clean(entryData['languageCode']);

        final headword =
            clean(entryData['text']);

        final entryType =
            clean(entryData['entryType']) ??
            'word';

        final primaryScriptCode =
            clean(
        ...
        var entry =
            await DictionaryEntry.db.findFirstRow(
          session,
          where: (e) =>
        ...
        if (entry == null) {
          entry =
              await DictionaryEntry.db.insertRow(
            session,
            DictionaryEntry(
        ...
        if (rawExamples is List) {
          for (
            ...
            example ??=
                await DictionaryExample.db.insertRow(
              session,
              DictionaryExample(
                entryId: entryId,
            ...
            for (final rawText
              ...
              final existingText =
                  await DictionaryExampleText.db
                      .findFirstRow(
                session,
                where: (t) =>
                    t.exampleId.equals(
