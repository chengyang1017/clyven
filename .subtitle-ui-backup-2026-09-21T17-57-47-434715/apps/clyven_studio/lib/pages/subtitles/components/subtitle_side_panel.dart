...
class SubtitleSidePanel extends StatelessComponent {
  ...
  Component build(BuildContext context) {
    ...
    final script = scriptCode == null ? null : ScriptConfig.findByCode(scriptCode!);

    return aside(
      classes: 'subtitle-editor-side subtitle-inspector-v2',
      [
          ...
          div(classes: 'subtitle-inspector-card', [
            ...
            strong([
              .text(
                script == null
                    ? '未指定'
                    : language?.scriptNameOf(script.code, 'zh') ?? script.nameOf('zh'),
              ),
            ]),
            if (script != null)
        ...
        div(classes: 'subtitle-inspector-tip', [
          strong([.text('多文字模式')]),
          p([
            .text('切换文字不会复制时间轴。不同文字表示挂在同一个 cue 下。'),
          ]),
        ]),
      ],
    );
