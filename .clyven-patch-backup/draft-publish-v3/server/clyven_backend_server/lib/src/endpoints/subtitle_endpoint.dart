...
class SubtitleEndpoint extends Endpoint {
  ...
  Future<void> _ensurePublishBaseline(Session session, int trackId) async {
    ...
    final payload = cue == null ? null : await _buildPublishedPayload(session, track);
    final now = DateTime.now();
  ...
  Future<void> _markDraftChanged(Session session, int trackId) async {
    ...
    if (state == null) throw Exception('Failed to create subtitle publish state.');
  ...
  Future<String> _buildPublishedPayload(Session session, SubtitleTrack track) async {
    ...
    final texts = cueIds.isEmpty
        ? <SubtitleCueText>[]
        : await SubtitleCueText.db.find(session, where: (r) => r.cueId.inSet(cueIds));
  ...
  Future<List<SubtitleCueDetail>> _publishedDetailsFromPayload(
    ...
    for (final row in tokens) {
      if (selectedScriptCode != null &&
          row.scriptCode != null &&
          row.scriptCode != selectedScriptCode) continue;
    ...
    return [
      for (final cue in cues)
        SubtitleCueDetail(
          cue: cue,
          texts: cue.id == null ? [] : textsByCue[cue.id!] ?? [],
          tokens: cue.id == null ? [] : tokensByCue[cue.id!] ?? [],
          phrases: cue.id == null ? [] : phrasesByCue[cue.id!] ?? [],
          karaokeSegments: cue.id == null ? [] : karaokeByCue[cue.id!] ?? [],
        ),
    ];
  }
  ...
  int? _snapshotInt(dynamic value) => value == null ? null : (value as num).toInt();
