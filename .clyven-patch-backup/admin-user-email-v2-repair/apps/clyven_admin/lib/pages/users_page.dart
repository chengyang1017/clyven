    ...
    return _users
        .where((user) {
          return user.authorName.toLowerCase().contains(query) ||
              user.authorId.toLowerCase().contains(query);
        })
  ...
  Component build(BuildContext context) {
    ...
    return div(classes: 'admin-users-page', [
      div(classes: 'admin-users-toolbar', [
        div([
          h2([.text('Users')]),
          p([.text('按 Clyven authorId 查看用户，以及该用户账号名下的全部视频。')]),
        ]),
        button(
      ...
      if (_loading)
        ...
        div(classes: 'admin-users-grid', [
          section(classes: 'admin-users-column', [
            ...
            input<String>(
              type: InputType.text,
              attributes: {'placeholder': '搜索用户名 / authorId', 'value': _search},
