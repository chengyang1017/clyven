...
class _WatchPageState extends State<WatchPage> {
  ...
  int? _primaryLanguageTrackId;
  int? _secondaryLanguageTrackId;
  ...
  Future<void> _load() async {
    try {
      ...
      setState(() {
        ...
        _primaryLanguageTrackId = primary?.track.id;
        _comments = comments;
        _loading = false;
      });
    } catch (error) {
  ...
  Future<void> _selectPrimary(_SubtitleSelection selection) async {
    ...
    setState(() {
      _primary = selection;
      _primaryCues = cues;
      _primaryLanguageTrackId = selection.track.id;
  ...
  Future<void> _selectSecondary(_SubtitleSelection selection) async {
    ...
    setState(() {
      ...
      _secondaryCues = cues;
      _secondaryLanguageTrackId = selection.track.id;
      _subtitlesEnabled = true;
    });
  }
  ...
  Component _settingsPanel() {
    return div(classes: 'watch-player-settings-layer', [
      div(classes: 'watch-subtitle-settings-card', [
        ...
        _subtitleSelectionSection(title: '主字幕', secondary: false),
        _subtitleSelectionSection(title: '第二字幕', secondary: true),
      ]),
    ]);
  }

  Component _subtitleSelectionSection({
    required String title,
    required bool secondary,
  }) {
    final current = secondary ? _secondary : _primary;

    final selectedLanguageTrackId = secondary
        ? (_secondaryLanguageTrackId ?? current?.track.id)
        : (_primaryLanguageTrackId ?? current?.track.id);

    final selectedLanguage = _availabilityForTrackId(selectedLanguageTrackId);

    return section(classes: 'watch-subtitle-levels', [
      div(classes: 'watch-subtitle-section-title', [
        h3([.text(title)]),
        if (secondary && _secondary != null)
          button(
            type: ButtonType.button,
            ...
            onClick: () {
              setState(() {
                _secondary = null;
                _secondaryCues = const [];
                _secondaryLanguageTrackId = null;
              });
            },
            [.text('关闭')],
          ),
      ...
      div(classes: 'watch-subtitle-level-label', [
        span(classes: 'watch-subtitle-level-number', [.text('1')]),
        strong([.text('语言')]),
      ]),

      div(classes: 'watch-language-tabs', [
        for (final item in _availability)
          button(
            type: ButtonType.button,
            classes:
                'watch-language-option'
                '${selectedLanguageTrackId == item.track.id ? ' is-active' : ''}',
            onClick: () {
              setState(() {
                if (secondary) {
                  _secondaryLanguageTrackId = item.track.id;
                } else {
                  _primaryLanguageTrackId = item.track.id;
                }
              });
            },
            [
              span(classes: 'watch-language-option-name', [
                .text(_languageLabel(item)),
              ]),
              span(classes: 'watch-language-option-code', [
                .text(item.track.languageCode),
              ]),
              if (item.track.variantCode != null &&
                  item.track.variantCode!.trim().isNotEmpty)
                span(classes: 'watch-language-option-variant', [
                  .text(item.track.variantCode!),
                ]),
            ],
          ),
      ]),
      ...
      if (selectedLanguage != null) ...[
        div(classes: 'watch-subtitle-level-label watch-script-level-label', [
          span(classes: 'watch-subtitle-level-number', [.text('2')]),
          strong([.text('文字')]),
          span(classes: 'watch-subtitle-level-current', [
            .text(_languageLabel(selectedLanguage)),
          ]),
        ]),
        div(classes: 'watch-script-options watch-script-level', [
          for (final scriptCode in selectedLanguage.scriptCodes)
            button(
              type: ButtonType.button,
              classes:
                  'watch-script-option'
                  '${current?.track.id == selectedLanguage.track.id && current?.scriptCode == scriptCode ? ' is-active' : ''}'
                  '${secondary && _primary?.track.id == selectedLanguage.track.id && _primary?.scriptCode == scriptCode ? ' is-disabled' : ''}',
              onClick: () {
        ...
        div(classes: 'watch-subtitle-level-empty', [.text('先选择语言')]),
    ]);
  }
