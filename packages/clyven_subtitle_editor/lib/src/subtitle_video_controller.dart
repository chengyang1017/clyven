...
class SubtitleVideoController {
  SubtitleVideoController({required this.onSeek});
  ...
  html.VideoElement? get _videoElement {
    final element = html.document.getElementById('subtitle-video-player');
  ...
  void startReversePlayback() {
    ...
    _reversePlaybackTimer = Timer.periodic(const Duration(milliseconds: 100), (
      _,
    ) {
  ...
  void keepTimelinePlayheadVisible({
    ...
    final timeline = html.document.querySelector('.subtitle-timeline');
    ...
    timeline.scrollLeft = targetScroll.clamp(0.0, maxScroll).round();
  }

  void centerTimelineAt({required int milliseconds, required int durationMs}) {
    final timeline = html.document.querySelector('.subtitle-timeline');
    ...
    timeline.scrollLeft = (pointX - viewportWidth / 2)
        .clamp(0.0, maxScroll < 0 ? 0.0 : maxScroll)
        .round();
  }
