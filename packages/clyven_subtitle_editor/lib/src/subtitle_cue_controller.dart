...
class SubtitleCueController {
  ...
  Future<void> saveCue(SubtitleCueDetail detail) async {
    ...
    try {
      final updated = await service.updateText(cueId: cueId, text: text);
  ...
  Future<void> saveTiming(SubtitleCueDetail detail) async {
    final cueId = detail.cue.id;
    ...
    final startText =
        editedStartTimes[cueId] ?? formatSubtitleTime(detail.cue.startMs);

    final endText =
        editedEndTimes[cueId] ?? formatSubtitleTime(detail.cue.endMs);
  ...
  Future<void> deleteCue({
    ...
    try {
      await service.delete(cueId: cueId);
  ...
  Future<void> nudgeCue({
    ...
    final cueDuration = detail.cue.endMs - detail.cue.startMs;

    final newStart = (detail.cue.startMs + deltaMs)
        .clamp(0, durationMs - cueDuration)
        .toInt();
