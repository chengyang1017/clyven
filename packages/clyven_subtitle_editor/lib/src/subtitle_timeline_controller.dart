...
class SubtitleTimelineController {
  ...
  void seekFromTimelineClick(dynamic event) {
    ...
    final relativeX = (mouseX - rect.left).clamp(0, rect.width).toDouble();
  ...
  void beginScrubPlayhead(dynamic event) {
    ...
    moveSubscription = html.document.onMouseMove.listen((moveEvent) {
      seekFromMouse(moveEvent);
    });

    upSubscription = html.document.onMouseUp.listen((_) async {
